...
class ListeSimplementChaineeTriee:
    ...
    def __init__(self, valeur_sentinelle, nombres: range | None = None):
        ...
        ajoute.
        """
        self.valeur_sentinelle = valeur_sentinelle
        self.tete = None

        if nombres is not None:
            iterateur = iter(nombres)
            self.tete = queue = Cellule(next(iterateur), None)

            for k in iterateur:
                queue.suivant = suivant = Cellule(k, None)
                queue = suivant

            queue.suivant = self.tete
            queue.valeur = self.valeur_sentinelle
    ...
    def __iter__(self):
        ...
        while suivant.valeur != self.valeur_sentinelle:
            yield suivant

            if suivant.suivant is not None:

...
def _draw_none(none_inst, show_address: bool, dot_file):
    ...
    print(
        f'<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" BGCOLOR="{NONE_BACKGROUND}">',
        file=dot_file,
    )
    if show_address:
...
def _draw_callable(callable_inst, show_address: bool, dot_file):
    ...
    print(f"  {dot_name} [label=<", file=dot_file)
    print(
        f'<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" BGCOLOR="{INSTANCE_BACKGROUND}">',
        file=dot_file,
    )
    if show_address:
        print(f"<TR><TD>0x{id(callable_inst):x}</TD></TR>", file=dot_file)
    print(
        f"<TR><TD><u>{type(callable_inst).__name__}</u></TD></TR>", file=dot_file)
...
def _draw_primitive(inst, show_address: bool, dot_file):
    ...
    print(f"  {dot_name} [label=<", file=dot_file)
    print(
        f'<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" BGCOLOR="{CINSTANCE_BACKGROUND}">',
        file=dot_file,
    )
    if show_address:
...
def _draw_range(range_inst, show_address: bool, dot_file):
    ...
    if show_address:
        print(
            f'<TR><TD COLSPAN="3">0x{id(range_inst):x}</TD></TR>', file=dot_file)
    print(
        f'<TR><TD COLSPAN="3"><u>{type(range_inst).__name__}</u></TD></TR>',
        file=dot_file,
    )
    print(f"<TR><TD>start</TD><TD>stop</TD><TD>step</TD></TR>", file=dot_file)
    print(
        f'<TR><TD>{getattr(range_inst, "start")}</TD>'
...
def _draw_list_tuple(seq_inst, dot_file, deeply, show_address, seen_insts):
    ...
    if seq_inst:
        if show_address:
            print(
                f'<TR><TD COLSPAN="{len(seq_inst)}">0x{id(seq_inst):x}</TD></TR>',
                file=dot_file,
            )
        print(
            f'<TR><TD COLSPAN="{len(seq_inst)}"><u>{type(seq_inst).__name__}</u></TD></TR>',
            file=dot_file,
        )
        ...
        for elem in seq_inst:
            if not deeply and _is_primitive(elem):
                ...
                if show_address:
                    print(
                        f'<TD PORT="port_child{child_count}">0x{id(elem):x}</TD>',
                        file=dot_file,
                    )
                else:
                    print(
                        f'<TD PORT="port_child{child_count}">{DOT_CHAR}</TD>',
                        end="",
                        file=dot_file,
                    )
    ...
    # Sinon elle est vide
        ...
        print(
            f"<TR><TD><u>{type(seq_inst).__name__}</u></TD></TR>", file=dot_file)
        print(f"<TR>", end="", file=dot_file)
...
def _draw_dict(dict_inst, dot_file, deeply, show_address, seen_insts):
    ...
    print(
        f'<TR><TD COLSPAN="{len(dict_inst)*2}"><u>{type(dict_inst).__name__}</u></TD></TR>',
        file=dot_file,
    )
    child_to_be_drawn = []
    child_count = 0
        ...
        for elem, elem_name in ((key, "key"), (value, "value")):
            if not deeply and isinstance(elem, (int, float, str)):
                ...
                if show_address:
                    ...
                    print(
                        f'<TD PORT="port_child{child_count}">{elem_name} {DOT_CHAR}</TD>',
                        end="",
                        file=dot_file,
                    )
                child_to_be_drawn.append(elem)
...
def _draw_other(inst, dot_file, deeply, show_address, seen_insts):
    ...
    if nb_attributes:
        print(
            f'<TR><TD COLSPAN="{nb_attributes}"><u>{type(inst).__name__}</u></TD></TR>',
            file=dot_file,
        )
        child_count = 0
        print(f"<TR>", file=dot_file)
        ...
        for field_name in vars(inst):
            ...
            if not deeply and _is_primitive(field_obj):
                elem_str = (
                    f'"{field_obj}"' if isinstance(
                        field_obj, str) else f"{field_obj}"
                )
                print(f"<TD>{elem_str}</TD>", end="", file=dot_file)
            else:
                if show_address:
                    print(
                        f'<TD PORT="port_child{child_count}">0x{id(field_obj):x}</TD>',
                        file=dot_file,
                    )
                else:
                    print(
...
def _draw_instance(
    ...
    # Drawing here depends on deeply
        ...
        dot_name = _draw_list_tuple(
            inst, dot_file, deeply, show_address, seen_insts)
    ...
    # Handle any other instance in a generic way
        ...
        dot_name = _draw_other(inst, dot_file, deeply,
                               show_address, seen_insts)
...
def _draw_variable(
    ...
    print(
        f'<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" BGCOLOR="{VARIABLE_BACKGROUND}">',
        file=dot_file,
    )
    print(f"<TR><TD>{variable.name}</TD></TR>", file=dot_file)
    if show_address:
        print(
            f'<TR><TD PORT="port_{variable.name}">0x{id(variable.value):x}</TD></TR>',
            file=dot_file,
        )
    else:
        print(
            f'<TR><TD PORT="port_{variable.name}">{DOT_CHAR}</TD></TR>', file=dot_file
        )
    print("</TABLE>>];", file=dot_file)
    ...
    print(
        f"struct_{variable.name}:port_{variable.name} -> {inst_dot_name};",
        file=dot_file,
    )


def _draw_stack(
    variables: typing.Iterable[Variable], show_address: bool, dot_file: typing.TextIO
):
...
def display_stack_and_heap(
    variables: typing.Iterable[Variable],
    deeply: bool = True,
    show_address: bool = False,
    visualize: bool = True,
    image_name: str = None,
    ...
    for variable in variables:
        print(
            f"stack:port_{id(variable)} -> struct_{id(variable.value)};", file=dot_f)
...
def trace(
    ...
    def parametrized_tracer(function):
        ...
        def tracer(*args, **kwargs):
            ...
            posargs_str = ", ".join(
                [f"{arg.name}={arg.value}" for arg in arguments])
            # Varargs tuple
            ...
            if args[first_vararg_index:]:
                arguments.append(
                    Variable(
                        inspect.getfullargspec(function).varargs,
            ...
            allargs_str = ", ".join(
                filter(None, (posargs_str, varargs_str, kwargs_str))
            )
